...
const Messages = () => {
  ...
  const fetchSuggestions = async (search = "",skip = 0) => {
    ...
    if(search === "" && fetchBool)
    {
      res = await fetch(`https://partners-in-crime.herokuapp.com/users/data?skip=${skip}`, {
        headers:{
    ...
    {
      res = await fetch(`https://partners-in-crime.herokuapp.com/searchmessage?q=${search}`,{
        headers:{
          authentication: `Bearer ${token}`
        }
      })
  ...
  const UpdateMessagesUserData = async (id) => {
    setLoading(true)
    let userData = {}
    let data = await fetch("https://partners-in-crime.herokuapp.com/auth/getUser", {
      method: 'POST',
      headers: {
