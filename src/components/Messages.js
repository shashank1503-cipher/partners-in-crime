...
const Messages = () => {
  ...
  const fetchSuggestions = async (search = "",skip = 0) => {
    ...
    if(search === "" && fetchBool)
    {
      res = await fetch(`https://anplt2s03b.execute-api.ap-south-1.amazonaws.com/dev/users/data?skip=${skip}`, {
        headers:{
    ...
    {
      res = await fetch(`https://anplt2s03b.execute-api.ap-south-1.amazonaws.com/dev/searchmessage?q=${search}`,{
        headers:{
          authentication: `Bearer ${token}`
        }
      })
